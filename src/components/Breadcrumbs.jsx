...
const Breadcrumbs = () => {
    ...
    const breadcrumbsList = pathnames.map((pathname, index) => {
        ...
        return {
            name: `${pathname === 'tournaments' ? 'Tournaments' : pathname === 'freefire' ? 'Free Fire' : pathname === 'pubgmobile' ? 'PUBG Mobile' : pathname === 'announcement' ? 'Announcements' : pathname === 'games' ? 'Games' :pathname === 'feedback' ? 'Feedback' :pathname}`,
            url: routeTo,
            active: isLast,
        };
    });
