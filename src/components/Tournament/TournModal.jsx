...
export const TournModal = ({ tournament }) => {
    const gameDetails = JSON.parse(tournament.gameDetails)
    let totalPrize = 0;
    ...
    const handleFinalJoin = async () => {
        if (user) {
            setProgress(30)
            try {
                const tourn = await database.getDocument(db_id, 'tournaments', tournament.$id, [Query.select(['max', 'participants', 'entryFee'])])
