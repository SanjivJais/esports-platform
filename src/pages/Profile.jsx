    ...
    <div className='flex flex-col w-full max-w-[1280px] self-center p-6'>
      <div className="flex">
        <div className="flex gap-4">
          <div className=''>
            <img src="/icons/dummyProfilePic.png" alt="" className='md:h-[100px] md:w-[100px] h-[70px] w-[70px] rounded-[50%]' />
            <div className='bg-secondaryLight md:p-2 p-1 rounded-[50%] md:text-sm text-[12px] h-fit w-fit relative bottom-7 md:left-16 left-14 hover:cursor-pointer' ><FaCamera /></div>
          </div>
          <div className="flex flex-col gap-1 md:mt-4 mt-2">
            <h3 className='text-offBlue font-bold md:text-2xl text-xl'>{user.name}</h3>
            <label htmlFor="user email" className='text-inactive'>{user.email}</label>
          </div>
        </div>
      </div>
      ...
      <div className='mb-6'>
        <div className="flex max-md:justify-between md:gap-8 gap-4 md:text-base text-sm custom-scrollbar overflow-x-auto">
          <label htmlFor="" onClick={(e) => handleTabs(e)} className={`profileTab md:text-base text-[12px] ${activeTab === 0 ? 'border-b-2 border-primary' : 'text-inactive hover:text-offBlue'}  pb-2 font-bold cursor-pointer`}>Game Profiles</label>
          <label htmlFor="" onClick={(e) => handleTabs(e)} className={`profileTab md:text-base text-[12px] ${activeTab === 1 ? 'border-b-2 border-primary' : 'text-inactive hover:text-offBlue'}  pb-2 font-bold cursor-pointer`}>Load Balance</label>
          <label htmlFor="" onClick={(e) => handleTabs(e)} className={`profileTab md:text-base text-[12px] ${activeTab === 2 ? 'border-b-2 border-primary' : 'text-inactive hover:text-offBlue'}  pb-2 font-bold cursor-pointer`}>Withdraw</label>
          <label htmlFor="" onClick={(e) => handleTabs(e)} className={`profileTab md:text-base text-[12px] ${activeTab === 3 ? 'border-b-2 border-primary' : 'text-inactive hover:text-offBlue'}  pb-2 font-bold cursor-pointer`}>Account Details</label>
        </div>
            ...
            <div className="flex w-full justify-between">
              <div className="flex flex-col gap-6">
                <div className='flex flex-col gap-[2px]'>
                  <label htmlFor="user name" className='text-inactive'>Name</label>
                  <strong className='text-offBlue'>{user.name}</strong>
                </div>
                <div className='flex flex-col gap-[2px]'>
                  <label htmlFor="user name" className='text-inactive'>Email</label>
                  <strong className='text-offBlue'>{user.email}</strong>
                </div>
                <div className='flex flex-col gap-[2px]'>
                  <label htmlFor="user name" className='text-inactive'>Joined On</label>
